...
- 极致精简：绿色便携版，解压即用，无需安装。
- 并发 Worker：自动根据 CPU 与内存调整推理并发数，充分利用硬件。

## 本 fork 的技术改造

本仓库是原版 MagicMirror 的深度改造 fork，最核心的变化是**将推理服务从 Python 整体重写为纯 Rust**：

- 🦀 **纯 Rust 推理服务**：单二进制 `server.exe`，零环境依赖，无需安装 Python/CUDA。
- ⚡ **毫秒级启动**：配合 ONNX Runtime DirectML 后端，普通 CPU 即可流畅运行。
- 🧩 **并发 Worker 系统**：模型内存共享、只加载一次，并发数自动计算。
- 🔄 **子进程托管**：桌面端自动派生/清理 `server.exe`，无控制台窗口，退出无残留。
- 📦 **绿色便携版**：解压即用，可离线批量换脸。
